...
if [[ -f $TIMER ]]; then
  ...
  if [[ "$STATUS" != "STOPPED" ]]; then
    ...
    TASK_DESCRIPTION=$(eval "zenity --entry --width=500 --title 'Start Timer ($(basename $TIMER_DIRECTORY))' --text 'Enter Task Description:' --ok-label='Start' $(~/bin/timer -D "$TIMER_DIRECTORY" --report task --start-date $(date +'%Y-%m-%d' -d '3 months ago') --end-date $(date +'%Y-%m-%d' -d 'next month') | awk -F "\t" '{ gsub(/[ \t]+$/, "", $1); print $1 }' | sort -u | (tasks="";while read t; do tasks="$tasks '$t'"; done;echo $tasks))")
