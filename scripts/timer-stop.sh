...
if [[ -f $TIMER ]]; then
  ...
  if [[ "$STATUS" == "RUNNING" ]] || [[ "$STATUS" == "PAUSED" ]]; then
    ...
    if [[ -f ~/.timer/timers ]]; then
      NEW_TIMER_DIRECTORY=$(zenity --list --column="Timer" --column="Directory" $(names=""; while read t; do names="$names `basename $t` $t"; done < ~/.timer/timers;echo $names) --width=500 --height=400 --print-column=2 --text="Choose Timer:" --title="Timer")
